...
CREATE PROCEDURE GET_INSTANCES
AS
BEGIN
...
CREATE PROCEDURE GET_INSTANCE_PROPERTY_VALUES (@INSTANCE_NAME VARCHAR(100))
AS
BEGIN
	...
	INNER JOIN TB_INSTANCE INST ON APP.APPLICATION_ID = INST.APPLICATION_ID 
	INNER JOIN TB_INSTANCE_PROPERTY INSTPROP ON INST.INSTANCE_ID = INSTPROP.INSTANCE_ID
											AND PROP.PROPERTY_ID = INSTPROP.PROPERTY_ID
	WHERE INST.ST_ENABLE = 1
	AND (@INSTANCE_NAME IS NULL OR INST.INSTANCE_NAME = @INSTANCE_NAME)
	UNION
